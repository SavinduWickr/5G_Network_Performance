...
clustered_dir = r"G:\Ai Projects\low_lgbm"
os.makedirs(clustered_dir, exist_ok=True)
kmeans_clusters = 4
sample_size = 5000  # smaller sample for visual clarity
clustered_csv = os.path.join(clustered_dir, f'kmeans_{sample_size//1000}k_scaled.csv')
...
pd.DataFrame([scores1]).to_csv(os.path.join(clustered_dir, "scores_without_cluster_scaled.csv"))
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "forecast_actual_vs_predicted_without_cluster_scaled.png"))
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "error_distribution_no_cluster_scaled.png"))
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "hourly_error_no_cluster_scaled.png"))
plt.show()
...
pd.DataFrame([scores2]).to_csv(os.path.join(clustered_dir, "scores_with_cluster_scaled.csv"))
plt.figure(figsize=(12, 5))
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "forecast_actual_vs_predicted_with_cluster_scaled.png"))
plt.show()
...
plt.ylabel("Frequency")
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "error_distribution_with_cluster_scaled.png"))
plt.show()
...
plt.ylabel("Mean Absolute Percentage Error")
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "hourly_error_with_cluster_scaled.png"))
plt.show()

# Final comparison
scores_df = pd.DataFrame([scores1, scores2], index=['Without_Cluster', 'With_Cluster'])
scores_df.to_csv(os.path.join(clustered_dir, "lgbm_comparison_scaled.csv"))
