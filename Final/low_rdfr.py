...
clustered_dir = r"G:\Ai Projects\low_rdfr"
os.makedirs(clustered_dir, exist_ok=True)
kmeans_clusters = 4
sample_size = 5000
clustered_csv = os.path.join(clustered_dir, f'rf_kmeans_5k_scaled.csv')
...
df1 = df.copy()
train1 = df1.iloc[:int(len(df1)*0.7)]
test1 = df1.iloc[int(len(df1)*0.7):]
...
print("\n--- Training Random Forest WITHOUT cluster label ---")
...
pd.DataFrame([scores1]).to_csv(os.path.join(clustered_dir, "rf_scores_without_cluster_scaled.csv"))
...
plt.title("Random Forest: Actual vs. Predicted svr1 (Without Cluster)")
plt.xlabel("Datetime")
plt.ylabel("svr1")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "rf_forecast_actual_vs_predicted_without_cluster_scaled.png"))
plt.show()

# Error plots for Iteration 1
error_pct1 = np.abs((y1_test - y1_pred) / np.where(y1_test == 0, 1, y1_test)) * 100
plt.figure(figsize=(7, 4))
plt.hist(error_pct1, bins=30)
plt.title("Random Forest Forecast Percentage Error Distribution (No Cluster)")
plt.xlabel("Error %")
plt.ylabel("Frequency")
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "rf_error_distribution_no_cluster_scaled.png"))
plt.show()
...
plt.title("Random Forest Forecast Error by Hour (No Cluster)")
plt.ylabel("Mean Absolute Percentage Error")
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "rf_hourly_error_no_cluster_scaled.png"))
plt.show()

# ---- Apply KMeans and Train Iteration 2 ----
df2 = df.copy()
...
train2 = df2.iloc[:int(len(df2)*0.7)]
test2 = df2.iloc[int(len(df2)*0.7):]
...
print("\n--- Training Random Forest WITH cluster label ---")
...
pd.DataFrame([scores2]).to_csv(os.path.join(clustered_dir, "rf_scores_with_cluster_scaled.csv"))
plt.figure(figsize=(12, 5))
...
plt.title("Random Forest: Actual vs. Predicted svr1 (With Cluster)")
plt.xlabel("Datetime")
plt.ylabel("svr1")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "rf_forecast_actual_vs_predicted_with_cluster_scaled.png"))
plt.show()

error_pct2 = np.abs((y2_test - y2_pred) / np.where(y2_test == 0, 1, y2_test)) * 100
plt.figure(figsize=(7, 4))
plt.hist(error_pct2, bins=30)
plt.title("Random Forest Forecast Percentage Error Distribution (With Cluster)")
plt.xlabel("Error %")
plt.ylabel("Frequency")
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "rf_error_distribution_with_cluster_scaled.png"))
plt.show()
...
plt.title("Random Forest Forecast Error by Hour (With Cluster)")
plt.ylabel("Mean Absolute Percentage Error")
plt.tight_layout()
plt.savefig(os.path.join(clustered_dir, "rf_hourly_error_with_cluster_scaled.png"))
plt.show()

# Final comparison
scores_df = pd.DataFrame([scores1, scores2], index=['Without_Cluster_RF', 'With_Cluster_RF'])
scores_df.to_csv(os.path.join(clustered_dir, "rf_comparison_scaled.csv"))
print("\nRandom Forest Model Comparison Table:\n", scores_df)
